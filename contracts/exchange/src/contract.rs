...
use cosmwasm_std::{to_binary, Binary, Deps, DepsMut, Env, MessageInfo, Response, StdResult, Decimal, Addr};
...
pub fn instantiate(
    ...
    let state = State {
        ...
        exchange_rate: msg.exchange_rate.clone(),
...
pub fn execute(
    ...
    let api = deps.api;
    match msg {
        ExecuteMsg::SetOwner { owner } => execute::set_owner(deps, info, api.addr_validate(owner.clone().as_str())?),
        ExecuteMsg::SetExchangeRate { exchange_rate } => execute::set_exchange_rate(deps, info, exchange_rate),
...
pub mod execute {
    ...
    pub fn trade(deps: DepsMut<ProvenanceQuery>, env: Env, info: MessageInfo) -> Result<Response<ProvenanceMsg>, ContractError> {
    ...
    pub fn set_exchange_rate(deps: DepsMut<ProvenanceQuery>, info: MessageInfo, exchange_rate: Decimal) -> Result<Response<ProvenanceMsg>, ContractError> {
        ...
        STATE.update(deps.storage, |mut state| -> Result<_, ContractError> {
            ...
            state.exchange_rate = exchange_rate.clone();
            Ok(state)
        })?;
    ...
    pub fn set_owner(deps: DepsMut<ProvenanceQuery>, info: MessageInfo, owner: Addr) -> Result<Response<ProvenanceMsg>, ContractError> {
        let previous_owner = STATE.load(deps.storage)?.owner.clone();
...
pub mod query {
    use crate::msg::{GetOwnerResponse, GetExchangeInfoResponse, GetMarkerAccountResponse};
    ...
    pub fn get_marker_account(deps: Deps<ProvenanceQuery>) -> StdResult<GetMarkerAccountResponse> {
        let state = STATE.load(deps.storage)?;
        Ok(GetMarkerAccountResponse { marker_account: state.marker_account.to_string() })
    }

    pub fn get_exchange_info(deps: Deps<ProvenanceQuery>) -> StdResult<GetExchangeInfoResponse> {
        let state = STATE.load(deps.storage)?;
        Ok(GetExchangeInfoResponse { native_denom: state.native_denom, private_denom: state.private_denom, exchange_rate: state.exchange_rate.clone() })
    }
}
...
mod tests {
    ...
    use crate::msg::{GetOwnerResponse, GetMarkerAccountResponse, GetExchangeInfoResponse};

    use super::*;
    use cosmwasm_std::testing::{mock_env, mock_info};
    use cosmwasm_std::{from_binary, Uint128, Attribute, Coin, BankMsg};
    use cosmwasm_std::CosmosMsg::Bank;
    use provwasm_mocks::mock_dependencies;
    use provwasm_std::{mint_marker_supply, withdraw_coins, transfer_marker_coins, burn_marker_supply};
    ...
    fn proper_initialization() {
        ...
        assert_eq!(Attribute::new("action", "provwasm.contracts.exchange.init"), res.attributes[0]);
        assert_eq!(Attribute::new("integration_test", "v1"), res.attributes[1]);
        assert_eq!(Attribute::new("owner", "tp1w9fnesmguvlal3mp62na3f58zww9jtmtwfnx9h"), res.attributes[2]);
        ...
        assert_eq!(Attribute::new("exchange_rate", Decimal::from_atomics(Uint128::new(10), 1).unwrap().to_string()), res.attributes[5]);
        assert_eq!(Attribute::new("marker_account", "tp15h7xkfj4v549sfdu0gxl9wltc85lywhjdxt6xu"), res.attributes[6]);
        ...
        assert_eq!("tp15h7xkfj4v549sfdu0gxl9wltc85lywhjdxt6xu", value.marker_account);
        ...
        assert_eq!(Decimal::from_atomics(Uint128::new(10), 1).unwrap(), value.exchange_rate);
    }
    ...
    fn proper_set_owner() {
        ...
        let info = mock_info("tp1w9fnesmguvlal3mp62na3f58zww9jtmtwfnx9h", &[]);
        let msg = ExecuteMsg::SetOwner { owner: "tp15h7xkfj4v549sfdu0gxl9wltc85lywhjdxt6xu".to_string() };
        let res = execute(deps.as_mut(), mock_env(), info, msg).unwrap();

        // Verify the attributes
        assert_eq!(0, res.messages.len());
        assert_eq!(4, res.attributes.len());
        assert_eq!(Attribute::new("action", "provwasm.contracts.exchange.set_owner"), res.attributes[0]);
        assert_eq!(Attribute::new("integration_test", "v1"), res.attributes[1]);
        assert_eq!(Attribute::new("previous_owner", "tp1w9fnesmguvlal3mp62na3f58zww9jtmtwfnx9h"), res.attributes[2]);
        assert_eq!(Attribute::new("new_owner", "tp15h7xkfj4v549sfdu0gxl9wltc85lywhjdxt6xu"), res.attributes[3]);

        // Verify the new owner
        let res = query(deps.as_ref(), mock_env(), QueryMsg::GetOwner {}).unwrap();
        let value: GetOwnerResponse = from_binary(&res).unwrap();
        assert_eq!("tp15h7xkfj4v549sfdu0gxl9wltc85lywhjdxt6xu".to_string(), value.owner);
    ...
    fn proper_set_exchange_rate() {
        ...
        let info = mock_info("tp1w9fnesmguvlal3mp62na3f58zww9jtmtwfnx9h", &[]);
        let msg = ExecuteMsg::SetExchangeRate { exchange_rate: Decimal::from_atomics(Uint128::new(20), 1).unwrap() };
        let res = execute(deps.as_mut(), mock_env(), info, msg).unwrap();
        assert_eq!(0, res.messages.len());
        assert_eq!(4, res.attributes.len());
        assert_eq!(Attribute::new("action", "provwasm.contracts.exchange.set_exchange_rate"), res.attributes[0]);
        assert_eq!(Attribute::new("integration_test", "v1"), res.attributes[1]);
        assert_eq!(Attribute::new("previous_exchange_rate", Decimal::from_atomics(Uint128::new(10), 1).unwrap().to_string()), res.attributes[2]);
        assert_eq!(Attribute::new("new_exchange_rate", Decimal::from_atomics(Uint128::new(20), 1).unwrap().to_string()), res.attributes[3]);
        ...
        assert_eq!(Decimal::from_atomics(Uint128::new(20), 1).unwrap(), value.exchange_rate);
        ...
        // Verify only owner can set
        let info = mock_info("tp1w9fnesmguvlal3mp62na3f58zww9jtmtwfnx9h", &[]);
        let msg = ExecuteMsg::SetExchangeRate { exchange_rate: Decimal::from_atomics(Uint128::new(0), 0).unwrap() };
        let res = execute(deps.as_mut(), mock_env(), info, msg);
        match res {
            Err(ContractError::InvalidExchangeRateError {}) => {}
    ...
    fn proper_trade() {
        ...
        let info = mock_info("tp1w9fnesmguvlal3mp62na3f58zww9jtmtwfnx9h", &[Coin::new(1, "denom1"), Coin::new(1, "denom2")]);
        let msg = ExecuteMsg::Trade{};
        let res = execute(deps.as_mut(), mock_env(), info, msg);
        match res {
            Err(ContractError::InvalidFundsError {}) => {}
        ...
        let info = mock_info("tp1w9fnesmguvlal3mp62na3f58zww9jtmtwfnx9h", &[Coin::new(1, "denom3")]);
        let msg = ExecuteMsg::Trade{};
        let res = execute(deps.as_mut(), mock_env(), info, msg);
        match res {
        ...
        // Error with invalid denom
        let info = mock_info("tp1w9fnesmguvlal3mp62na3f58zww9jtmtwfnx9h", &[Coin::new(0, "denom1")]);
        let msg = ExecuteMsg::Trade{};
        let res = execute(deps.as_mut(), mock_env(), info, msg);
        match res {
        ...
        let info = mock_info("tp1w9fnesmguvlal3mp62na3f58zww9jtmtwfnx9h", &[Coin::new(10, "denom1")]);
        let msg = ExecuteMsg::Trade{};
        let res = execute(deps.as_mut(), mock_env(), info, msg).unwrap();
        assert_eq!(3, res.messages.len());
        assert_eq!(4, res.attributes.len());
        assert_eq!(Attribute::new("action", "provwasm.contracts.exchange.trade"), res.attributes[0]);
        assert_eq!(Attribute::new("integration_test", "v1"), res.attributes[1]);
        assert_eq!(Attribute::new("send", Coin::new(10, "denom1").to_string()), res.attributes[2]);
        assert_eq!(Attribute::new("receive", Coin::new(5, "denom2").to_string()), res.attributes[3]);

        let mint = mint_marker_supply(5 as u128, "denom2").unwrap();
        let withdraw = withdraw_coins("denom2", 5 as u128, "denom2", Addr::unchecked("tp1w9fnesmguvlal3mp62na3f58zww9jtmtwfnx9h")).unwrap();
        let transfer = transfer_marker_coins(5 as u128, "denom2", Addr::unchecked("tp1w9fnesmguvlal3mp62na3f58zww9jtmtwfnx9h"), mock_env().contract.address).unwrap();
        ...
        let info = mock_info("tp1w9fnesmguvlal3mp62na3f58zww9jtmtwfnx9h", &[Coin::new(10, "denom2")]);
        let msg = ExecuteMsg::Trade{};
        let res = execute(deps.as_mut(), mock_env(), info, msg).unwrap();
        ...
        assert_eq!(Attribute::new("action", "provwasm.contracts.exchange.trade"), res.attributes[0]);
        assert_eq!(Attribute::new("integration_test", "v1"), res.attributes[1]);
        assert_eq!(Attribute::new("send", Coin::new(10, "denom2").to_string()), res.attributes[2]);
        assert_eq!(Attribute::new("receive", Coin::new(20, "denom1").to_string()), res.attributes[3]);
